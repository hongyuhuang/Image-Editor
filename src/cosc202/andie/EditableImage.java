...
class EditableImage {
    ...
    private Object opsMarker;
    private Object reOpsMarker;
    ...
    private EditableImage(Stack<ImageOperation> newOps, Stack<ImageOperation> newRedoOps) {
        original = null;
        current = null;
        ops = newOps;
        redoOps = newRedoOps;
    ...
    public void startMacroRec() {
        ...
        if(redoOps.isEmpty()) {
            ...
            reOpsMarker = redoOps.peek();
        }
    }

    public void endMacroRec(String imageFilepath) throws Exception{
        Stack<ImageOperation> newOps = new Stack<ImageOperation>();
        Stack<ImageOperation> newReOps = new Stack<ImageOperation>();
        for( int i = 0; i < ops.size() && ops.peek() != opsMarker; i ++) {
            newOps.push(ops.pop());
        }
        for( int i = 0; i < redoOps.size() && redoOps.peek() != reOpsMarker; i ++) {
            newReOps.push(redoOps.pop());
        }
        EditableImage macroImage = new EditableImage( newOps, newReOps);
        macroImage.saveMacroAs(imageFilepath);
    }

    public void saveMacroAs(String imageFilepath) throws Exception {
        this.opsFilename = imageFilepath + ".ops";
        // Write image file based on file extension
        //String extension = imageFilename.substring(1+imageFilename.lastIndexOf(".")).toLowerCase();
        //ImageIO.write(original, extension, new File(imageFilename));
        // Write operations file
